...
-- Didn't like just using ship_address, so we changed it to a combination of the full address --
SELECT (ship_address, ship_city, ship_region, ship_postal_code, ship_country) AS ship_destination, COUNT(*) AS ship_count FROM orders
GROUP BY ship_destination
...
WITH number_unique_shipping_companies AS 
(
    SELECT customer_id, COUNT(DISTINCT ship_via) AS unique_shippers FROM orders
    GROUP BY customer_id
)
SELECT TRUNC(AVG(unique_shippers),3) FROM number_unique_shipping_companies;
...
SELECT r.region_description, t.territory_description, e.last_name, e.first_name FROM employees e
...
SELECT us.state_name, us.state_abbr, c.company_name FROM customers c
RIGHT JOIN us_states us ON us.state_abbr = c.region
...
SELECT t.territory_description, e.title_of_courtesy, e.last_name FROM employees e
JOIN employee_territories et ON e.employee_id = et.employee_id
RIGHT JOIN territories t ON et.territory_id = t.territory_id
...
SELECT c.company_name, SUM(od.quantity) AS total_products FROM customers c
